...
{{-- ================================================================
     ...
     ================================================================ --}}
<div id="excelImportModal"
     style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5);
            z-index:2000; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:8px; max-width:480px; width:92%;
                padding:24px; box-shadow:0 10px 40px rgba(0,0,0,0.25);">
        <h5 class="mb-2">
            <i class="fas fa-file-excel text-success"></i>
            Import Excel — Existing Items Found
        </h5>
...
    // ================= EXCEL IMPORT =================
    ...
    // Holds the selected Excel file while the user chooses
    // whether to replace existing items, append items, or cancel.
    let pendingExcelFile = null;

    // Check whether the table contains actual item data.
    ...
    $('#excel_import').on('change', function(e) {
        ...
        // If the edit invoice already has real items, ask what to do.
        if (tableHasRealData()) {
            showExcelImportModal();
        } else {
            // Empty table: import directly.
            runExcelImport(pendingExcelFile, 'append');
        }
    });

    // Remove existing items and replace them with Excel items.
    $('#excelImportReplace').on('click', function() {
        hideExcelImportModal();

        if (pendingExcelFile) {
            runExcelImport(pendingExcelFile, 'replace');
        }
    });

    // Keep existing items and add Excel items after them.
    ...
    // Cancel the import without changing the invoice items.
    ...
    // Existing Excel import logic, wrapped only with the selected mode.
    function runExcelImport(file, mode) {
        ...
        reader.onload = function(e) {
            ...
            // REPLACE MODE:
            // Remove all existing item rows and their associated parts rows.
            if (mode === 'replace') {
                $('#PurchaseTable tr.item-row, #PurchaseTable tr.parts-row').remove();
            }

            // APPEND MODE:
            // Only remove the completely blank starter row, if one exists.
            // Existing invoice items are preserved.
            if (mode === 'append') {
                let firstRow = $('#PurchaseTable tr.item-row').first();

                if (
                    $('#PurchaseTable tr.item-row').length === 1 &&
            ...
            jsonData.forEach(row => {
                ...
                if (row['Item Name'] && row['Item Name'].toString().trim() !== '') {
                    ...
                    const purityRaw =
                        row['Purity'] !== undefined && row['Purity'] !== ''
                            ? row['Purity']
                            : 0.92;

                    const purityMatched = setPurityDropdown(
                        currentItemRow.find('.purity'),
                        purityRaw
                    );

                    if (!purityMatched) {
                        unmatchedPurityRows.push({
                            item: row['Item Name'],
                            purity: purityRaw
                        });
                    }
                    ...
                    // Existing calculation function — unchanged.
                    recalcItemGrossWeight(currentItemRow);
                }

                if (
                    row['Part Name'] &&
                    row['Part Name'].toString().trim() !== '' &&
                    currentItemRow
                ) {
                    ...
                    // Existing calculation trigger — unchanged.
                    currentPartRow.find('.part-qty').trigger('input');
                }
            });

            // Existing totals calculation — unchanged.
            calculateTotals();
            ...
            if (unmatchedPurityRows.length > 0) {
                ...
                alert(
                    'Items imported, but the following rows had a Purity value with no matching option in the Purity dropdown — please check them manually:\n\n' +
                    list
                );
            } else {
                alert(
