...
<div class="row">
  <div class="col">
    <form id="return-form" action="{{ route('purchase_return.store') }}" method="POST">
      @csrf
      ...
      <section class="card">
        <header class="card-header"><h2 class="card-title">New Purchase Return</h2></header>
        <div class="card-body">

          {{-- ===================== HEADER ===================== --}}
          <div class="row mb-5">
            <div class="col-md-3">
              <label class="fw-bold">Select Purchase Invoice <span class="text-danger">*</span></label>
              <select name="purchase_invoice_id" id="purchase_invoice_id" class="form-control select2-js" required>
                <option value="">Select Invoice</option>
                @foreach ($invoices as $invoice)
                  <option value="{{ $invoice->id }}"
                    data-vendor-id="{{ $invoice->vendor_id }}"
                    data-vendor-name="{{ $invoice->vendor->name ?? '' }}">
          ...
          <section class="card">
            ...
            <div class="table-responsive">
              <div id="no_invoice_msg" class="p-3 text-muted">
                <i class="fas fa-info-circle"></i> Please select a purchase invoice above to load its items.
              </div>
          ...
          <div class="row mb-3 d-none" id="bank_transfer_fields">
            ...
            <div class="col-md-2"><label>Vendor Bank</label><input type="text" name="transfer_to_bank" class="form-control"></div>
...
<script>
  $(document).ready(function () {
      ...
      $('#purchase_invoice_id').on('change', function () {
          ...
          $.get(`/purchase-return/${invoiceId}/items`)
              .done(function (data) {
                  $('#loading_msg').hide();
                  ...
                  // ── Vendor ──
                  $('#vendor_display').val(inv.vendor_name);
                  $('#vendor_id').val(inv.vendor_id);
                  ...
                  invoiceItems.forEach(function (item, i) {
                      ...
                      const returnedBadge = isReturned
                          ? '<span class="badge bg-danger ms-1">Returned</span>'
                          : '';
                      ...
                      <tr class="${rowClass}" data-index="${i}">
                          <td class="text-center">
                              <input type="checkbox"
                                    class="item-select form-check-input"
                                    data-index="${i}"
                                    ${isReturned ? 'disabled title="This item has already been returned"' : ''}>
                          </td>
                          <td class="fw-semibold">
                              ${escHtml(item.item_name || '-')}${returnedBadge}
                          </td>
                          <td>${escHtml(item.item_description || '-')}</td>
                          <td>${fmtNum(item.purity, 3)}</td>
                          <td>${fmtNum(item.net_weight, 3)}</td>
                          ...
                          <td class="text-center">
                              ${hasParts
                                  ? `<button type="button" class="btn btn-sm btn-primary toggle-parts-btn" data-index="${i}">
                      ...
                      // ── Parts sub-table (collapsed by default) ──
                      if (hasParts) {
                          let partsHtml = `
                          ...
                          item.parts.forEach(function (part) {
                              partsHtml += `
                                          <tr>
                                              ...
                                              <td>${fmtNum(part.certification_charges || 0, 2)}</td>
                          ...
                          // ── Hide AFTER append — inline style on <tr> stripped by browser ──
                          $(`#parts-row-${i}`).hide();
                      }
                  });
      ...
      // ── Select All / Deselect All — skips disabled (already returned) checkboxes ──
      ...
      function calculateSummary() {
          ...
          $('.item-select:checked').each(function () {
              ...
              Object.entries(scalarFields).forEach(function ([field, value]) {
                  $('#return-form').append(
              ...
              if (item.parts && item.parts.length > 0) {
                  item.parts.forEach(function (part, j) {
                      const partFields = {
                          ...
                          'certification_charges': part.certification_charges || 0,
                          'total':                 part.total                 || 0,
                      };

                      Object.entries(partFields).forEach(function ([field, value]) {
                          $('#return-form').append(
                              $('<input>').attr({
                                  type:  'hidden',
                                  name:  `items[${itemIndex}][parts][${j}][${field}]`,
                                  value: value ?? 0,
                              })
                          );
                      });
